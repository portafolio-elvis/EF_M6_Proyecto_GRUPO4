...
app.post("/mover-tarjeta", (req, res) => {
  ...
  if (!fromBoard || !toBoard)
    return res.status(400).json({ ok: false, error: "Board no encontrado" });

  const fromList = fromBoard.lists.find((l) => l.id === parseInt(fromListId));
  const toList = toBoard.lists.find((l) => l.id === parseInt(toListId));

  if (!fromList || !toList)
    return res.status(400).json({ ok: false, error: "Lista no encontrada" });

  const cardIndex = fromList.cards.findIndex((c) => c.id === parseInt(cardId));
  if (cardIndex === -1)
    return res.status(400).json({ ok: false, error: "Tarjeta no encontrada" });
